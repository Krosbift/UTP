...
class vector {
  ...
  public:
    ...
    vector& operator=(const vector &vector_copy) {
      if (this != &vector_copy) {
        delete[] this.pointer_vector;

        this.capacity = vector_copy.capacity;
        this.elements = vector_copy.elements;

        this.pointer_vector = new T[this.capacity];
        for (size_t i = 0; i < this.elements; ++i) {
          this.pointer_vector[i] = vector_copy.pointer_vector[i];
        }
      }
      return *this;
    }
    ...
    T at(size_t index) const {
      ...
      return this.pointer_vector[index];
    }
    ...
    void push_back(const T &value) {
      if (this.elements == this.capacity) {
        ...
        T* new_pointer_vector = new T[this.capacity];
        for (size_t i = 0; i < this.elements; ++i) {
          new_pointer_vector[i] = this.pointer_vector[i];
        }

        delete[] this.pointer_vector;
        this.pointer_vector = new_pointer_vector;
      }
      this.pointer_vector[this.elements++] = value;
    }
    ...
    void pop_back() {
      ...
      if ((this.elements + 10) == this.capacity) {
        this.capacity -= 5;

        T* new_pointer_vector = new T[this.capacity];
        for (size_t i = 0; i < (this.elements - 1); i++) {
          new_pointer_vector[i] = this.pointer_vector[i];
        }
